...
class JPL_TeacherStudent_Datalodaer(DataLoader):
    def __init__(self, dataset, batch_size, student_sequence_length, student_frame_sample_hop, num_workers,
                 drop_last=True, shuffle=False):
        super(JPL_TeacherStudent_Datalodaer, self).__init__(dataset=dataset, batch_size=batch_size, shuffle=shuffle,
                                                            drop_last=drop_last, num_workers=num_workers)
        self.student_sequence_length = student_sequence_length
        self.student_frame_sample_hop = student_frame_sample_hop
        self.body_l_pair_num = len(body_l_pair)
        self.head_l_pair_num = len(head_l_pair)
        self.hand_l_pair_num = len(hand_l_pair)
        self.collate_fn = self.teacher_student_collate_fn

    def teacher_student_collate_fn(self, data):
        teacher_input, int_label, att_label, act_label = [], [], [], []
        x_tensors_list = [torch.zeros(
            (len(data) * int(self.student_sequence_length / self.student_frame_sample_hop) * body_point_num, 3)),
            torch.zeros((len(data) * int(
                self.student_sequence_length / self.student_frame_sample_hop) * head_point_num, 3)),
            torch.zeros((len(data) * int(
                self.student_sequence_length / self.student_frame_sample_hop) * hands_point_num, 3))]
        edge_index_list = [torch.zeros((2, len(data) * int(
            self.student_sequence_length / self.student_frame_sample_hop) * (
                                                2 * self.body_l_pair_num + body_point_num))).to(torch.int64),
                           torch.zeros((2, len(data) * int(
                               self.student_sequence_length / self.student_frame_sample_hop) * (
                           ...
                           torch.zeros((2, len(data) * int(
                               self.student_sequence_length / self.student_frame_sample_hop) * (
                                                2 * self.hand_l_pair_num + hands_point_num))).to(torch.int64)]
        batch = [torch.zeros(
            (len(data) * int(self.student_sequence_length / self.student_frame_sample_hop) * body_point_num)).to(
            torch.int64), torch.zeros(
            len(data) * int(self.student_sequence_length / self.student_frame_sample_hop) * head_point_num).to(
            torch.int64), torch.zeros(
            (len(data) * int(self.student_sequence_length / self.student_frame_sample_hop) * hands_point_num)).to(
            torch.int64)]
        point_nums = [body_point_num, head_point_num, hands_point_num]
        edge_nums = [2 * (self.body_l_pair_num) + (body_point_num), 2 * self.head_l_pair_num + head_point_num,
                     2 * self.hand_l_pair_num + hands_point_num]
        ...
        for index, d in enumerate(data):
            if index == 0:
                for iii in range(len(d[0])):
                    if type(d[0][iii]) != int:
                        teacher_input.append(torch.zeros((len(data), 3, d[0][iii].shape[1], d[0][iii].shape[2], 1)))
                        teacher_input[iii][0] = torch.Tensor(d[0][iii])
            else:
                for iii in range(len(d[0])):
                    if type(d[0][iii]) != int:
                        teacher_input[iii][index] = torch.Tensor(d[0][iii])
            for ii in range(int(self.student_sequence_length / self.student_frame_sample_hop)):
                for i in range(len(d[0])):
                    if i == 0:
                        edge_index = torch.Tensor(body_l_pair).t()
                    elif i == 1:
            ...
            att_label.append(d[1][1])
            act_label.append(d[1][2])
        return (teacher_input, (x_tensors_list, edge_index_list, batch)), (
            torch.Tensor(int_label), torch.Tensor(att_label), torch.Tensor(act_label))
