...
class GNN(torch.nn.Module):
    def __init__(self, is_coco, body_part, data_format, framework, model, max_length, attention):
        ...
        if self.model in ['gnn_keypoint_lstm', 'gnn_keypoint_conv1d']:
            ...
            # self.pool1 = SAGPooling(self.keypoint_hidden_dim * (self.num_heads if self.attention else 1),
            #                          ratio=self.pooling_rate)
            # self.pool2 = SAGPooling(self.keypoint_hidden_dim * (self.num_heads if self.attention else 1),
            #                          ratio=self.pooling_rate)
            # self.pool3 = SAGPooling(self.out_channels, ratio=self.pooling_rate)
